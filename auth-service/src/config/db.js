...
const connectDB = async () => {
  ...
  if (!isValidMongoUri(uri)) {
    console.error('Invalid or missing MONGODB_URI for auth-service');
    process.exit(1);
  }

  mongoose.connection.on('connected', () => console.log('Auth MongoDB connected'));
  mongoose.connection.on('error', (error) => console.error('Auth MongoDB error:', error.message));
  mongoose.connection.on('disconnected', () => console.warn('Auth MongoDB disconnected'));
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt += 1) {
    try {
      ...
      console.error(`Auth MongoDB connection attempt ${attempt} failed:`, error.message);
